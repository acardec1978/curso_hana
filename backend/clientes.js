...
appClientes.get("/", (req, res) => {
  var conn = req.db;
  conn.exec('SELECT * FROM "CURSO_ACARDEC.db::Vendas.Clientes"', function(err, resultado) {
  	if (err) {
...
appClientes.post("/", (req, res) => {
	...
	var conn = req.db;
	conn.exec(`INSERT INTO "CURSO_ACARDEC.db::Vendas.Clientes" ("codigo", "nome") values (${novoCliente.codigo},'${novoCliente.nome}')`, (err) => {
		if (err) {
	  		res.status(500);
	  		res.json(err);
	  	} else {
...
appClientes.put("/:codigo", (req, res) => {
	...
	var conn = req.db;
	conn.exec(`UPDATE "CURSO_ACARDEC.db::Vendas.Clientes" set "codigo" = ${clienteAlterado.codigo}, "nome" = '${clienteAlterado.nome}' where "codigo" = ${codigo}`, (err, resultado) => {
		if (err) {
...
appClientes.delete("/:codigo", (req, res) => {
	var codigo = req.params.codigo;
	var conn = req.db;
	conn.exec(`DELETE FROM "CURSO_ACARDEC.db::Vendas.Clientes" where "codigo" = ${codigo}`, (err, resultado) => {
		if (err) {
	  		res.status(500);
	  		res.json(err);
	  	} if (resultado === 0) {
